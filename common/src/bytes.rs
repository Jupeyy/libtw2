...
pub trait AsBytesExt: ByteArray + zerocopy::IntoBytes + zerocopy::Immutable {
...
pub trait FromBytesExt:
    ByteArray + zerocopy::FromBytes + zerocopy::Immutable + zerocopy::KnownLayout
{
        ...
        Self: Sized,
    {
        ...
        Some((Self::ref_from_bytes(result).unwrap(), rest))
    }
    fn ref_from_array(bytes: &Self::ByteArray) -> &Self
    where
        Self: Sized,
    {
        Self::ref_from_bytes(bytes.as_ref()).unwrap()
    }
...
impl<T: ByteArray + zerocopy::IntoBytes + zerocopy::Immutable> AsBytesExt for T {}
impl<T: ByteArray + zerocopy::FromBytes + zerocopy::Immutable + zerocopy::KnownLayout> FromBytesExt
    for T
{
}
